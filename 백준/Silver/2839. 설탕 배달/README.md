...
### 풀이
```
가장 합리적인 케이스를 찾는 문제, 매순간 5로 나누어 떨어지는 것을 목표로 케이스를 탐색한다.
5로 나누어 떨어지는 경우 5로 나눈 몫이 답이다.
5로 나누어 떨어지지 않는 경우 3을 빼가면서 5로 나누어 떨어지는 경우를 찾는다.
3을 빼가다가 0보다 작아지는 경우 -1을 출력한다.
```
